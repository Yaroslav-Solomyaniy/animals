        ...
        <article className="mx-auto grid max-w-336 gap-5 rounded-[28px] border border-orange-100 bg-white p-3 shadow-[0_28px_90px_rgba(15,23,42,0.10)] transition hover:border-orange-200 hover:shadow-[0_32px_100px_rgba(242,116,56,0.14)] lg:grid-cols-[minmax(0,1fr)_380px]">
          <div className="relative min-h-[440px] overflow-hidden rounded-[24px] ring-1 ring-orange-100/70 transition hover:ring-primary/45 lg:min-h-[560px]">
            {article.image ? (
              ...
              <div className="absolute inset-0 bg-[linear-gradient(135deg,#0f172a_0%,#1e3a5f_20%,#155e75_38%,#4c1d95_58%,#be185d_78%,#f27438_100%)]" />
            )}
            {article.image && (
              <>
          ...
          <aside className="flex min-h-full flex-col justify-between rounded-[24px] border border-orange-100 bg-[linear-gradient(180deg,#fff7ed,#ffffff_52%)] p-5 shadow-[0_18px_55px_rgba(242,116,56,0.10)] transition hover:border-orange-200 hover:shadow-[0_24px_70px_rgba(242,116,56,0.16)] sm:p-6 lg:min-h-[560px] lg:p-7">
            ...
            <div className="mt-5 grid gap-3">
              <InfoLine icon={Calendar} label="Дата публікації" value={article.date} />
              <InfoLine
                icon={Clock3}
      ...
      return (
        <figure className="overflow-hidden rounded-[28px] border border-orange-100 bg-white shadow-soft transition hover:border-orange-200 hover:shadow-[0_20px_55px_rgba(242,116,56,0.14)]">
          <LightboxImage
            ...
            imageClassName="max-h-[600px] w-full object-contain"
          />
          {block.caption && (
        ...
        <div className="columns-2 gap-4 space-y-4">
          {block.images.map((image, index) => (
            <LightboxImage
              ...
              images={block.images}
              index={index}
              buttonClassName="group block w-full cursor-zoom-in break-inside-avoid overflow-hidden rounded-[24px] border border-orange-100 bg-white shadow-soft transition hover:-translate-y-0.5 hover:border-primary/50 hover:shadow-[0_18px_45px_rgba(242,116,56,0.16)]"
    ...
    case 'file':
      return (
        <a
          ...
          className="group flex items-center gap-4 rounded-2xl border border-slate-200 bg-white px-5 py-4 shadow-soft transition hover:border-primary hover:shadow-[0_8px_30px_rgba(242,116,56,0.12)]"
        >
          ...
          <div className="min-w-0 flex-1">
            <p className="truncate font-extrabold text-slate-800 group-hover:text-primary">
              {block.name || 'Завантажити файл'}
            </p>
            {block.description && (
              <p className="mt-0.5 text-sm text-slate-500">{block.description}</p>
            )}
          </div>
          <Download className="h-5 w-5 shrink-0 text-slate-300 transition group-hover:text-primary" />
        </a>
      )
  }
}
...
function getBlockWidthClass(block: NewsContentBlock) {
  ...
  switch (block.type) {
    ...
    case 'file':      return 'mx-auto max-w-2xl'
    default:          return ''
  }
}
