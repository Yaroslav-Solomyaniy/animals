...
import { CalendarHeart, Heart, PawPrint, ShieldCheck } from 'lucide-react'

import { LinkButton } from '@/components/ui/Button'
import { SITE_CONTACTS, SITE_NAME_CITY, SITE_NAME_MAIN, SITE_ROUTES } from '@/lib/site-config'
...
export default function HomeScreenCarousel() {
  ...
  return (
    <section className="contacts-gradient-coral relative px-5 py-5 outline-none focus-visible:ring-4 focus-visible:ring-primary/20 sm:px-6 sm:py-6 lg:px-8 lg:py-16">
      ...
      <div className="relative mx-auto grid w-full max-w-336 grid-rows-[minmax(0,1fr)_360px] overflow-hidden rounded-[28px] border border-gray-200 bg-white shadow-[0_30px_100px_rgba(31,41,55,0.1)] sm:grid-rows-[minmax(0,1fr)_420px] lg:grid-cols-[minmax(0,0.98fr)_minmax(420px,1.02fr)] lg:grid-rows-1">
        <div className="flex min-h-64 flex-col items-start justify-between gap-4 px-5 py-6 sm:min-h-80 sm:gap-5 sm:px-10 sm:py-10 lg:min-h-150 lg:px-14 lg:py-14 xl:p-16">
          ...
          <div>
            <h1 className="max-w-full text-left text-[28px] leading-[1.1] font-black  text-text-main sm:text-[34px] md:text-[42px] lg:text-[52px]">
              {SITE_NAME_MAIN}
              <span className="text-secondary"> {SITE_NAME_CITY}</span>
            </h1>
            <p className="mt-3 text-left text-sm leading-6 font-semibold text-gray-600 sm:mt-5 sm:text-base sm:leading-7 lg:max-w-xl lg:text-lg lg:leading-8">
          ...
          <div className={`mt-4 grid w-full gap-3 sm:mt-5 sm:gap-4 ${donationsEnabled ? 'grid-cols-1 sm:grid-cols-2' : 'grid-cols-1'}`}>
            <LinkButton href={SITE_ROUTES.animals} size="lg" className="flex h-12 w-full justify-center gap-2 rounded-xl px-5 text-sm">
      ...
      <section className="mt-8 sm:mt-10 lg:mt-12">
        <div className="mx-auto grid max-w-336 overflow-hidden rounded-[30px] border border-emerald-200 bg-[linear-gradient(135deg,#ecf8f1_0%,#f8fffb_54%,#fffaf0_100%)] grid-cols-1 grid-rows-[auto_320px_auto] sm:grid-cols-[minmax(0,0.65fr)_minmax(0,0.35fr)] sm:grid-rows-[1fr_auto] lg:min-h-83 lg:grid-cols-[minmax(0,0.58fr)_minmax(0,0.42fr)]">
          {/* Текст */}
          <div className="flex min-w-0 flex-col items-start p-6 pb-2 sm:p-7 sm:pb-4 lg:pt-9 lg:pr-6 lg:pl-10 lg:pb-4 xl:pl-12">
            ...
            <h2 className="mt-3 sm:mt-4 max-w-xl text-left text-[22px] font-black leading-tight text-text-main sm:text-[22px] lg:text-[46px] lg:leading-[1.4]">
              Запрошуємо на прогулянку з тваринами
            </h2>

            <p className="mt-2 max-w-lg text-left text-sm font-semibold leading-6 text-gray-600 lg:mt-5 lg:text-lg lg:leading-7">
          ...
          <div className="relative h-full sm:row-span-2">
            <Image
              src="/max.png"
              alt="Собака запрошує на прогулянку"
              fill
              unoptimized
              loading="lazy"
