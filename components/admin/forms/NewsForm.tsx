...
import { type NewsFormPayload, saveNewsPostAction } from '@/app/admin/news/actions'
import {
  clearNewsCoverAction,
  createNewsFileUploadAction,
  createNewsImageUploadAction,
  createNewsVideoUploadAction,
  deleteNewsImageAction,
  setNewsCoverAction,
} from '@/app/admin/news/media-actions'
import { VideoUploader } from '@/components/admin/forms/VideoUploader'
import { FileUploader } from '@/components/admin/forms/FileUploader'
...
import { emptyToNull, Field, IconTool, SmallAddButton, toDatetimeLocal } from '@/components/admin/forms/shared'
import {
  closestCenter,
  DndContext,
  type DragEndEvent,
...
export function NewsForm({ initial, initialAnimal, mode }: { initial?: NewsPostRow; initialAnimal?: AnimalWithPhoto; mode: 'create' | 'edit' }) {
  ...
  const [selectedAnimal, setSelectedAnimal] = useState<AnimalWithPhoto | null>(initialAnimal ?? null)
  const [isDirty, setIsDirty] = useState(false)
  const [isDraggingAny, setIsDraggingAny] = useState(false)
  const [blockKeys, setBlockKeys] = useState<string[]>(() => normalizeBlocks(initial?.content_blocks).map(() => uid()))
  const sensors = useSensors(
    useSensor(PointerSensor, { activationConstraint: { distance: 8 } }),
    useSensor(KeyboardSensor, { coordinateGetter: sortableKeyboardCoordinates })
  )

  // Track unsaved changes — skip first render and the run right after save
  ...
  async function uploadNewsImage(file: File) {
    ...
    try {
      ...
      setStatus('Upload заблоковано. Перевір CORS у R2.')
      return null
    }
    if (!response.ok) {
      setStatus(`R2 помилка: ${response.status}`)
      return null
    }
    return { ...upload, newsId: currentNewsId }
  }

  async function uploadNewsVideo(file: File) {
    const currentNewsId = newsId ?? (await createDraftForUpload())
    if (!currentNewsId) return null
    const upload = await createNewsVideoUploadAction({ newsId: currentNewsId, fileName: file.name, contentType: file.type })
    if (!upload.ok) {
      setStatus(upload.error)
      return null
    }
    ...
    return { r2Key: upload.r2Key, publicUrl: upload.publicUrl }
  }

  async function uploadNewsFile(file: File) {
    const currentNewsId = newsId ?? (await createDraftForUpload())
    if (!currentNewsId) return null
    const upload = await createNewsFileUploadAction({ newsId: currentNewsId, fileName: file.name, contentType: file.type || 'application/octet-stream' })
    if (!upload.ok) {
      setStatus(upload.error)
      return null
    }
  ...
  const addBlock = (type: NewsContentBlock['type']) => setBlocks((c) => [...c, createBlock(type)])

  const updateBlock = (index: number, patch: Partial<NewsContentBlock>) =>
    setBlocks((c) => c.map((b, i) => (i === index ? ({ ...b, ...patch } as NewsContentBlock) : b)))

  const removeBlock = (index: number) => setBlocks((c) => c.filter((_, i) => i !== index))
  ...
  function handleDragEnd(event: DragEndEvent) {
    ...
    setBlockKeys((k) => arrayMove(k, oldIndex, newIndex))
  }
  ...
  return (
    <div className="flex min-h-screen flex-col">
            ...
            <div className="flex items-center justify-between border-b border-slate-100 px-4 py-3">
              <div>
                ...
                <p className="text-xs text-slate-400">
                  {blocks.length} {blocks.length === 1 ? 'блок' : blocks.length < 5 ? 'блоки' : 'блоків'}
                </p>
              </div>
                    ...
                    <div className="space-y-3">
                      {blocks.map((block, index) => (
                        <BlockCard
                          ...
                          id={blockKeys[index] ?? String(index)}
                          block={block}
                          index={index}
                          total={blocks.length}
                          ...
                          onUploadVideo={uploadNewsVideo}
                          onUploadFile={uploadNewsFile}
            ...
            <div className="p-3">
              ...
              {form.cover_url ? (
                ...
                <button
                  ...
                  onClick={() => coverInputRef.current?.click()}
                  disabled={isCoverUploading}
                  className="flex w-full items-center justify-center gap-2 rounded-xl border border-dashed border-slate-200 py-3 text-xs font-bold text-slate-400 transition hover:border-orange-300 hover:bg-orange-50/50 hover:text-primary"
                >
                  {isCoverUploading ? <Loader2 className="h-4 w-4 animate-spin" /> : <ImagePlus className="h-4 w-4" />}
                  {isCoverUploading ? 'Завантаження...' : 'Додати обкладинку'}
                </button>
              )}
            </div>
          </div>
          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
            <label className="block space-y-2">
              ...
              <textarea
                ...
                className="block w-full border-0 bg-transparent p-0 text-sm leading-6 text-slate-700 placeholder:text-slate-300 outline-none focus:ring-0"
              />
            </label>
          </div>

          {/* Related animal */}
          <div className="rounded-2xl border border-orange-100 bg-white p-3 shadow-sm transition hover:border-orange-200">
            <p className="px-1 text-xs font-extrabold uppercase tracking-[0.16em] text-primary">Тварина з історії</p>
            {selectedAnimal ? (
              <div className="mt-3 overflow-hidden rounded-xl border border-slate-200">
                <div className="overflow-hidden rounded-[inherit] bg-white">
                  <div className="aspect-[3/4] w-full overflow-hidden bg-orange-50">
                    {selectedAnimal.photo_url ? (
                      <img src={selectedAnimal.photo_url} alt={selectedAnimal.name} className="h-full w-full object-cover" />
                    ) : (
                  ...
                  <div className="p-2.5">
                    <p className="truncate font-black leading-5 text-gray-950">{selectedAnimal.name || 'Без імені'}</p>
                    {selectedAnimal.approximate_age_label && (
                      <p className="mt-0.5 text-xs font-semibold text-gray-500">
                        {selectedAnimal.approximate_age_label} · {selectedAnimal.gender === 'female' ? 'Дівчинка' : 'Хлопчик'}
                      </p>
                    )}
                  </div>
                </div>
                <div className="flex border-t border-slate-100">
                  <button
                    ...
                    className="flex flex-1 items-center justify-center gap-1.5 py-2 text-xs font-bold text-primary transition hover:bg-orange-50"
                  >
                  ...
                  <button
                    ...
                    onClick={() => {
                      ...
                      setForm((c) => ({ ...c, related_animal_id: null }))
                    }}
                    className="flex w-10 items-center justify-center text-slate-300 transition hover:bg-red-50 hover:text-red-500"
                  >
              ...
              <button
                type="button"
                onClick={() => setIsAnimalModalOpen(true)}
                className="group mt-3 flex w-full flex-col items-center gap-2.5 rounded-xl border border-dashed border-orange-200 py-5 text-orange-300 transition hover:border-primary hover:bg-orange-50/60 hover:text-primary"
              >
                <span className="text-3xl">🐾</span>
                <span className="text-xs font-extrabold">Прив'язати тварину</span>
              </button>
            )}
          </div>
        </aside>
      ...
      <AnimalPickerModal
        ...
        onOpenChange={setIsAnimalModalOpen}
        selectedId={form.related_animal_id}
        onSelect={(animal) => {
          setSelectedAnimal(animal)
          setForm((c) => ({ ...c, related_animal_id: animal.id }))
        }}
      />
    </div>
  )
...
const BLOCK_ACTIONS = [
  ...
  { type: 'slider' as const, label: 'Карусель', icon: GalleryHorizontal },
...
function BlockCard({
  ...
  onUploadVideo: (file: File) => Promise<{ r2Key: string; publicUrl: string } | null>
  onUploadFile: (file: File) => Promise<{ r2Key: string; publicUrl: string } | null>
  ...
  return (
    <div ref={setNodeRef} style={style} className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
      <div className="flex items-center gap-2 bg-slate-50/80 px-3 py-2.5">
        <GripVertical className="h-4 w-4 shrink-0 cursor-grab text-slate-300 touch-none" {...attributes} {...listeners} />
        ...
        <div className="flex items-center gap-0.5">
          <IconTool label="Вище" disabled={index === 0} onClick={onMoveUp}>
            <ArrowUp className="h-3.5 w-3.5" />
          </IconTool>
          <IconTool label="Нижче" disabled={index === total - 1} onClick={onMoveDown}>
            <ArrowDown className="h-3.5 w-3.5" />
          </IconTool>
          <IconTool label="Видалити" onClick={() => void onRemove()} danger>
...
function BlockFields({
  ...
  switch (block.type) {
    case 'paragraph':
      return (
        <Textarea
          className="min-h-70 text-sm resize-y"
    ...
    case 'table':
      return (
        <div className="space-y-2">
          <Field label="Назва таблиці">
            <Input value={block.title ?? ''} onChange={(e) => onUpdate({ title: e.target.value })} />
          </Field>
          <div className="overflow-x-auto rounded-xl border border-slate-200">
            <table className="w-full border-collapse text-sm" style={{ minWidth: `${Math.max(300, block.columns.length * 140)}px` }}>
              <thead>
                <tr className="border-b-2 border-slate-200 bg-slate-50">
                  {block.columns.map((col, ci) => (
                    <th key={ci} className="group relative p-0">
                      <input
                        value={col}
                        onChange={(e) => onUpdate({ columns: block.columns.map((c, j) => (j === ci ? e.target.value : c)) })}
                        placeholder={`Колонка ${ci + 1}`}
                        className="w-full bg-transparent px-3 py-2 text-left text-xs font-extrabold text-slate-700 outline-none placeholder:text-slate-300 focus:bg-white"
                      />
                        ...
                        <button
                          ...
                          onClick={() =>
                            onUpdate({
                              ...
                              rows: block.rows.map((r) => r.filter((_, j) => j !== ci)),
                            })
                          }
                          className="absolute right-1 top-1/2 -translate-y-1/2 flex h-4 w-4 items-center justify-center rounded text-slate-300 opacity-0 transition group-hover:opacity-100 hover:bg-red-50 hover:text-red-500"
                        >
                  ...
                  <th className="w-8 border-l border-slate-200 p-0">
                    <button
                      type="button"
                      title="Додати колонку"
                      onClick={() =>
                        onUpdate({
                          columns: [...block.columns, `Кол. ${block.columns.length + 1}`],
                          rows: block.rows.map((r) => [...r, '']),
                        })
                      }
                      className="flex h-full w-full items-center justify-center py-2 text-slate-300 transition hover:bg-orange-50 hover:text-primary"
                    >
              ...
              <tbody>
                {block.rows.map((row, ri) => (
                  <tr key={ri} className="group border-b border-slate-100 last:border-0">
                    {block.columns.map((_, ci) => (
                      <td key={ci} className="p-0">
                        <input
                          value={row[ci] ?? ''}
                          onChange={(e) =>
                            onUpdate({ rows: block.rows.map((r, j) => (j === ri ? r.map((c, k) => (k === ci ? e.target.value : c)) : r)) })
                          }
                          placeholder="—"
                          className="w-full bg-transparent px-3 py-1.5 text-xs text-slate-700 outline-none placeholder:text-slate-200 focus:bg-orange-50/40"
                        />
                      </td>
                    ))}
                    <td className="w-8 border-l border-slate-200 p-0">
                      <button
                        type="button"
                        title="Видалити рядок"
                        onClick={() => onUpdate({ rows: block.rows.filter((_, j) => j !== ri) })}
                        className="flex h-full w-full items-center justify-center py-1.5 text-slate-200 opacity-0 transition group-hover:opacity-100 hover:bg-red-50 hover:text-red-500"
                      >
                        <X className="h-3 w-3" />
                      </button>
                    </td>
          ...
          <SmallAddButton onClick={() => onUpdate({ rows: [...block.rows, block.columns.map(() => '')] })}>+ Рядок</SmallAddButton>
        </div>
      )
        ...
        <div className="space-y-2">
          {block.buttons.map((btn, bi) => (
            <div key={bi} className="grid grid-cols-[1fr_1fr_120px_auto] gap-2 rounded-xl border border-slate-200 bg-slate-50 p-2.5">
              <Field label="Текст">
                <Input value={btn.label} onChange={(e) => onUpdate({ buttons: updBtn(block.buttons, bi, { label: e.target.value }) })} />
              </Field>
              <Field label="Посилання">
                <Input value={btn.href} onChange={(e) => onUpdate({ buttons: updBtn(block.buttons, bi, { href: e.target.value }) })} />
              </Field>
              <Field label="Стиль">
                <Select
                  value={btn.variant ?? 'primary'}
                  onChange={(e) => onUpdate({ buttons: updBtn(block.buttons, bi, { variant: e.target.value as 'primary' | 'outline' }) })}
                >
...
function ImageGrid({
  ...
  return (
    <div className={showAlt ? 'grid grid-cols-2 gap-3' : 'grid grid-cols-2 gap-2 sm:grid-cols-3'}>
      {images.map((img, i) => {
        ...
        return (
          <div key={i} className="flex flex-col gap-1.5">
            <div
              ...
              style={{ aspectRatio: showAlt ? '4/3' : '1/1' }}
            >
              {img.src ? (
                <>
      ...
      <div className="flex flex-col gap-1.5">
        <label
          className={`flex cursor-pointer flex-col items-center justify-center gap-1 rounded-xl border-2 border-dashed border-slate-200 text-slate-400 transition hover:border-orange-300 hover:text-primary ${showAlt ? 'aspect-[4/3]' : 'aspect-[4/3]'}`}
        >
...
function SingleImageUploader({
  ...
  return (
    <div className="group relative overflow-hidden rounded-xl border border-slate-200 bg-slate-100">
      {src ? (
        <>
          <img src={src} alt={alt} className="max-h-[480px] w-full object-contain" />
        ...
        <label className="flex h-40 w-full cursor-pointer flex-col items-center justify-center gap-2 text-slate-400 transition hover:text-primary">
